...
Use a Javascript library such as React to create the UI to leverage reusable components e.g climbing sessions on the feed and items within them, and in the user's profile. More data points for the user to log and being able to leverage those data points to show different areas of climbing progress e.g quantifying climbing descriptors and creating a chart to reflect patterns in climbing style. Maybe Google auth for making logging in more streamlined for users.

## Lessons Learned:

I enjoyed challenging myself with respect to creating charts using chart.js which I had never used previously. I also learned about using SASS with Bootstrap to easily customize variables. It was also efficient being able to shorten code significantly and reusing it by using partials. 